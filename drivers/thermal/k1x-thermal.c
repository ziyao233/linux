...
{
	struct k1x_thermal_sensor_desc *desc = thermal_zone_device_priv(tz);
...
static int k1x_thermal_probe(struct platform_device *pdev)
{
	...
	for (i = s->sr[0]; i <= s->sr[1]; ++i) {
		...
		if (IS_ERR(tzd))
			return dev_err_probe(dev, PTR_ERR(tzd), "faild to register sensor id %d\n", i);
